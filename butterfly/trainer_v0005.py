...
from .corpus.alignment_v0005 import MANIFEST, STAGE_FILES
from .registry import get_active_entry, get_entry, register_model

VERSION = "0.0005"
TRAINING_ROOT = ROOT / "training_state" / "v0.0005"
...
def _train_stage(
    ...
    *,
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        generator.manual_seed(5000 + sum(ord(c) for c in name) * 100 + epoch)
        loader = DataLoader(
            train_ds,
...
def train_v0005(preset="ryzen3600"):
    del preset  # v0.0005 inherits the exact accepted architecture; no resize is allowed.
    ensure_dirs()
    random.seed(5000)
    torch.manual_seed(5000)

    if not MANIFEST.exists():
        raise FileNotFoundError("Missing v0.0005 alignment corpus. Run 07_BUILD_ALIGNMENT_CORPUS_V0005.bat first.")
    for stage, paths in STAGE_FILES.items():
        for path in paths:
            if not path.exists():
                raise FileNotFoundError(f"Missing v0.0005 {stage} file: {path}")

    active = get_active_entry()
    if not active:
        raise RuntimeError("No active Butterfly brain to inherit from.")
    if active["version"] == VERSION:
        print("Butterfly v0.0005 is already active. There is nothing to retrain.")
        return MODELS_DIR / active["path"]
    if active["version"] != "0.0004":
        raise RuntimeError(
            f"v0.0005 was designed to inherit the accepted v0.0004 brain, but active is v{active['version']}. "
    ...
    if existing_candidate and not _read_progress():
        raise RuntimeError(
            "A registered v0.0005 candidate already exists. Compare/promote it or remove it through the normal rejection flow; "
    ...
    if progress:
        ...
        progress = None
        print("\nStarting v0.0005 FROM the accepted v0.0004 weights.")
    ...
    print("v0.0004 remains ACTIVE and untouched until v0.0005 passes the strict benchmark.")

    stage_defs = [
        # stage, epochs, lr, frozen lower blocks, patience
        ("basic_dialogue", 3, 6e-5, 3, 2),
        ("instruction_following", 3, 5e-5, 3, 2),
        ("epistemic_dialogue", 3, 4e-5, 3, 2),
        ("mixed_consolidation", 2, 2e-5, 0, 1),
    ]
    ...
    candidate_path = MODELS_DIR / "butterfly-v0.0005-candidate.safetensors"
    extra = {
        "candidate": True,
        "version": VERSION,
        "seed_brain": "0.0004",
        "training_objective": "assistant-only-supervised-alignment",
        "stages": stages,
    ...
    print("Next: run 09_COMPARE_AND_PROMOTE_V0005.bat.")
